---
title: 'NATS'
description: 'This health check verifies the ability to communicate with a [NATS server](https://nats.io/about/). It relies on `NATS.Client` package. Latest tag for the [official dockerhub image](https://hub.docker.com/_/nats/) is `2.6.6`.'
icon: 'undefined'
---
...
<Tabs>
  <Tab title=".NET CLI">
    ```bash
    dotnet add package AspNetCore.Pulse.Nats --version 8.0.3
    ```
  </Tab>
  <Tab title="Package Reference">
    ```xml
    <PackageReference Include="AspNetCore.Pulse.Nats" Version="8.0.3" />
    ```
  </Tab>
  <Tab title="Nuget">
    <a href="https://www.nuget.org/packages/AspNetCore.Pulse.Nats/" target="_blank">Go to nuget page</a>
  </Tab>
</Tabs>
